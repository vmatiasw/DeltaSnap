...
def test_iniciar_partida(repository, game, delta_db : DeltaDB):
    ...
    assert changes.get_frequency() == {'partidas': {'#table frequency': 1, 'iniciada': 1, 'inicio_turno': 1,
                                                    'duracion_turno': 1}, 'jugadores': {'#table frequency': 1, 'es_creador': 1, 'nombre': 1}}
    assert created.data == {('cartas', 5), ('cartas', 4),
                            ('cartas', 1), ('cartas', 3), ('cartas', 6), ('cartas', 2)}
    assert not deleted.data
    assert changes.data == {('partidas', 1): {'iniciada': (False, True), 'inicio_turno': ('0', '2021-10-10T10:00:00Z'), 'duracion_turno': (
        0, 60)}, ('jugadores', 1): {'es_creador': ("#column don't exist", True), 'nombre': ('Creador', "#column don't exist")}}
    assert created.remove_tables(['jugadores']).data == {(
        'cartas', 1), ('cartas', 2), ('cartas', 3), ('cartas', 4), ('cartas', 5), ('cartas', 6)}
    assert not deleted.remove_tables(['jugadores']).data
    assert changes.ignore_fields_changes({'partidas': ['iniciada']}).remove_tables(['jugadores']).data == {('partidas', 1): {
        'duracion_turno': (0, 60), 'iniciada': ('#change ignored', '#change ignored'), 'inicio_turno': ('0', '2021-10-10T10:00:00Z')}}
    assert changes.ignore_fields_changes({'partidas': ['iniciada']}).data == {('partidas', 1): {'iniciada': (
        '#change ignored', '#change ignored'), 'inicio_turno': ('0', '2021-10-10T10:00:00Z'), 'duracion_turno': (0, 60)}}
    assert created.get_schema() == {'cartas'}
    assert not deleted.get_schema()
    assert changes.get_schema() == {'partidas': {
        'iniciada', 'inicio_turno', 'duracion_turno'}}
